package rsa5

import (
	"crypto"
	"encoding/base64"
	"encoding/hex"
	"fmt"
	"testing"
	"runtime"
)


var privateKey2 = []byte(`
-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
`)

var publicKey = []byte(`
-----BEGIN PUBLIC KEY-----

iwIDAQAB
-----END PUBLIC KEY-----
`)

var  privateKey_test = []byte(`
-----BEGIN PRIVATE KEY-----

-----END PRIVATE KEY-----
`)
var data_test = `attach=附加信息&body=测试购买商品&charset=UTF-8&device_info=AND_SDK
&mch_app_id=com.wwl.tmgp.sgame&mch_app_name=玩玩乐&mch_create_ip=127.0.0.1&mch_id=175510359638&
nonce_str=1522309983261&notify_url=http://xxx/weixinwap-pay/testPayResult&out_trade_no=1522309969277
&service=pay.weixin.wappay&sign_type=RSA_1_256&total_fee=1&version=2.0`
var cipher Cipher

func init() {

	//client, err := NewDefault( string(privateKey2), string(publicKey))
	client, err := NewDefault( string(privateKey_test), string(publicKey))

	if err != nil {
		fmt.Println(Location(),err)
	}

	cipher = client
}

func Test_DefaultClient(t *testing.T) {

	cp, err := cipher.Encrypt([]byte("测试加密解密"))
	if err != nil {
		t.Error(Location(),err)
	}
	cpStr := base64.URLEncoding.EncodeToString(cp)

	fmt.Println(cpStr)

	ppBy, err := base64.URLEncoding.DecodeString(cpStr)
	if err != nil {
		t.Error(Location(),err)
	}
	pp, err := cipher.Decrypt(ppBy)

	fmt.Println(string(pp))
}

func Test_Sign_DefaultClient(t *testing.T) {

//	src := `attach=附加信息&body=测试购买商品&charset=UTF-8&device_info=AND_SDK
//&mch_app_id=com.wwl.tmgp.sgame&mch_app_name=玩玩乐&mch_create_ip=127.0.0.1&mch_id=175510359638&
//nonce_str=1522309983261&notify_url=http://xxx/weixinwap-pay/testPayResult&out_trade_no=1522309969277
//&service=pay.weixin.wappay&sign_type=RSA_1_256&total_fee=1&version=2.0`

   // srcutf8,err:= getByte(data_test,"UTF-8")
   // if err!=nil {
   // 	t.Fatal(err)
	//}
	signBytes, err := cipher.Sign([]byte(data_test), crypto.SHA256)
	if err != nil {
		t.Error(err)
	}
	fmt.Println("string(signBytes):",string(signBytes))
	fmt.Println("hex.EncodeToString(signBytes):",hex.EncodeToString(signBytes))
	fmt.Println("base64.StdEncoding.EncodeToString(signBytes):",base64.StdEncoding.EncodeToString(signBytes))


	sign := hex.EncodeToString(signBytes)
	fmt.Println(sign)

	signB, err := hex.DecodeString(sign)

	errV := cipher.Verify([]byte(data_test), signB, crypto.SHA256)
	if errV != nil {
		t.Fatal(errV)
	}
	fmt.Println("verify success")
}


func Location() string {
	_, f, line, _ := runtime.Caller(1)
	return fmt.Sprintf("[%s%d]", f, line)
}
