...
func (self *Client) ReadTcp() {
	...
	for {
		...
		log.Printf("buf length:%d,other tcp id:%d,length:%d",length,uint16(binary.LittleEndian.Uint16(buf[4:6])),uint16(binary.LittleEndian.Uint16(buf[6:8])))

		// msg_length := uint16(binary.LittleEndian.Uint16(buf[6:8]))
		// if int(msg_length + 8) > length {
...
func (self *Client) Write(data []byte) bool {
	...
	n,err := self.Conn.Write(frame)
	log.Print("=======================websocket write:",n,err)
	frame = []byte{0}
	return true
}
